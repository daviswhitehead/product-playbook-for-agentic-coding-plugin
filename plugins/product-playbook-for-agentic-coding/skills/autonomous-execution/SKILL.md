...
**When the metric has more than one producer path, the non-null read must come from the path under change.** A metric like "signups by channel" is fed by several code paths (header link, gate sheet, OAuth callback). An aggregate check — "at least one row resolves" — is satisfiable by any of them, including an old fixture from a path nobody touched. Slice the consuming query by the path (an `entry_type`, a surface, a source field) and read the value for **the path this task changed**; a standing guard over such a metric asserts per path, not in aggregate. *(chef-chopsky, 2026-09-18: a "≥1 attributed signup" guard stayed green for two months while the guest-first path — every real conversion — attributed nothing; the two attributed rows were pre-flight fixtures from the header link.)*
